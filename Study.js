...
if (hamibot.env.easyedge_ocr_url == "默认") {
...
function wht_update_tiku_competition() { // 是否更新题库
	threads.start(function() {
		//			console.info('查看使用须知，15s后自动关闭');
		var b = dialogs.build({
			title: "是否更新竞赛和挑战答题题库？",
...
if (shuangren == true || siren == true || 订阅 != 'a' || stronger != 'a' || tiaozhan) {
	...
	if (tiaozhan && !(siren == true || shuangren == true || 订阅 != 'a' || stronger != 'a')) {} //只开了挑战答题的话
		...
		if (choose == 'b') {
			...
			ocr_api(images.clip(captureScreen(), 0, Math.floor(device.height / 2), device.width, Math.floor(device
				.height / 2)));
...
// question_list[i][0], question_list[i][2], question, 音字
...
/* function similarity_pos(question, answer, q) {
	var num = 0;
	for (var i = 0; i < q.length; i++) {
 ...
 */
function init() {
	wht_update_tiku_competition();
	while (!wht_update_tiku_competition_end) {
		sleep(1000);
	};
	if (init_true) return;
	...
	if (update_tiku_competition) {
		...
		try {
			...
			if (!file_tmp) {
				download.interrupt();
				console.error('四人/双人/挑战题库加载超时！，再次加载一次');
				startDownload();
			}
			while (!file_tmp) {
				toastLog('等待加载四人/双人/挑战题库!!!');
				delay(2);
			}
			file_tmp = null;
			console.info('保存题库到手机根目录下的question.txt （路径：/sdcard/question.txt）');
			sleep(500);
			tikus = tikus.split('\n');
			...
			console.error('四人/双人/挑战题库获取失败，检查网络连接！！！');
			exit();
		}
	} else if (files.exists('/sdcard/question.txt')) {
		tikus = files.read('/sdcard/question.txt', encoding = "utf-8");
		tikus = tikus.split('\n');
		for (var i = 0; i < tikus.length; i++) {
			var t = tikus[i].split(' ');
			if (t[1] && t[0]) {
		...
		console.error('手机根目录下未找到question.txt （路径： /sdcard/question.txt）');
		exit();
	} else {
		console.error('未知错误！！！');
		exit();
		...
		try {
			...
			while (true) {
				...
				if (numread < 0) {
					...
					console.info("加载完成");
					try {
						files.write('/sdcard/question.txt', tikus, encoding = "utf-8");
					} catch (e) {
						console.error('题库写入文件失败');
						sleep(3000);
					}
					if (!files.exists('/sdcard/question.txt')) {
						console.error('题库写入未成功！');
						sleep(3000);
					}
...
function push_score() {
	...
	score += '\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n';
	try {
		url = 'https://pushplus.hxtrip.com/send?token=' + hamibot.env.Token.replace(/ /g, '') +
			'&title=Study&content=' + score + '&template=html';
		http.get(url);
	} catch (e) {}
	try {
		url = 'http://www.pushplus.plus/send?token=' + hamibot.env.Token.replace(/ /g, '') + '&title=Study&content=' +
			score + '&template=html';
		http.get(url);
	} catch (e) {}
}
